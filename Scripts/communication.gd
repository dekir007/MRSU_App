...
func _process(delta: float) -> void:
	if check_kb and OS.get_name() == "Android":
...
func parse_response(response : Array):
	for message_data in response:
		...
		message.sender_id = message_data["User"]["Id"]
		message.sender_name = message_data["User"]["FIO"]
		message.message = message_data["Text"] if message_data["Text"] != null else ""
		var date = message_data["CreateDate"].left(10).split("-")
		message.time = date[2] + "." + date[1] + "." + date[0] + " - " + Globals.get_hh_mm_from_date(message_data["CreateDate"])
...
func _on_send_btn_pressed() -> void:
	...
	# doesnt work, need content-type
	#CommunicationService.post(Globals.current_disc_id, text)
